...
export function claimTermsInText(theme: string | undefined, text: string) {
  const normalizedText = normalize(text);
  return FAMILIES.filter((family) => !theme || family.themes.includes(theme))
    .flatMap((family) => family.terms)
...
export function expandEquivalentClaimTerms(theme: string | undefined, observedTerms: string[]) {
  ...
  return { terms: [...expanded], families: families.map((family) => family.id) };
}
