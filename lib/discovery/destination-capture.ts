...
async function collectKeyword(keyword: string, theme: string): Promise<DestinationSerpResult[]> {
  ...
  try {
    const response = await fetch(url, {
      headers: {
        "user-agent": "Mozilla/5.0 VelvetPassportDiscovery/1.0",
    ...
    return parseRss(xml).slice(0, 10).flatMap((item, index) => {
      ...
      return [{
        ...
        domain,
        resultType: classifyDomain(domain),
        visibilityWeight: rankVisibility(index + 1),
...
function aggregateTheme(theme: string, results: DestinationSerpResult[]): DestinationThemeCapture {
  ...
  const topDomains = [...byDomain.entries()].map(([domain, data]) => ({
    domain,
    resultType: data.type,
    appearances: data.appearances,
    bestRank: data.bestRank,
    visibilityShare: totalWeight ? Math.round((data.weight / totalWeight) * 1000) / 10 : 0,
  })).sort((a, b) => b.visibilityShare - a.visibilityShare || a.bestRank - b.bestRank);

  return {
    theme,
    queryCount: new Set(relevant.map((result) => result.keyword)).size,
    resultCount: relevant.length,
    domains: topDomains.map((item) => ({
      domain: item.domain,
      resultType: item.resultType,
      visibilityShare: item.visibilityShare,
      status: "ESTIMATED",
      source: "BING_RSS_RANK_WEIGHT",
    })),
    topDomains: topDomains.slice(0, 10),
    status: relevant.length ? "ESTIMATED" : "UNKNOWN",
    source: "BING_RSS",
    observedAt: relevant[0]?.observedAt ?? new Date().toISOString(),
  };
}

export async function collectDestinationCapture(
  universe: KeywordUniverse = parisUncoveredUniverse,
  maxKeywordsPerTheme = 3,
) {
  const requests = universe.themes.flatMap(({ theme, keywords }) =>
    keywords.slice(0, maxKeywordsPerTheme).map((keyword) => collectKeyword(keyword, theme))
  );
