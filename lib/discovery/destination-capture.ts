...
export type DestinationSearchSource = "DUCKDUCKGO_HTML" | "DUCKDUCKGO_LITE" | "NONE";
...
export type DestinationSerpResult = {
  ...
  source: Exclude<DestinationSearchSource, "NONE">;
...
export type DestinationThemeCapture = {
  ...
  topDomains: Array<{ domain: string; resultType: DestinationResultType; appearances: number; bestRank: number; visibilityShare: number }>;
  status: EvidenceStatus;
  source: DestinationSearchSource;
  observedAt: string;
};

export type DestinationCaptureBudget = { maxKeywordsPerTheme?: number; maxTotalQueries?: number; };

const DOMAIN_TYPES: Array<[RegExp, DestinationResultType]> = [
  [/reddit\.com$/i, "FORUM"], [/tripadvisor\./i, "FORUM"], [/quora\.com$/i, "FORUM"],
  ...
  [/(google\.[a-z.]+|duckduckgo\.com)$/i, "MAP"],
];
...
function classifyDomain(domain: string): DestinationResultType { for (const [pattern, type] of DOMAIN_TYPES) if (pattern.test(domain)) return type; return "EDITORIAL"; }
function rankVisibility(rank: number) { const weights = [32, 20, 14, 10, 7, 5, 4, 3, 3, 2]; return weights[rank - 1] ?? 1; }
function normalizeDomain(url: string) { try { return new URL(url).hostname.toLowerCase().replace(/^www\./, ""); } catch { return ""; } }
...
async function collectKeyword(keyword: string, theme: string): Promise<DestinationSerpResult[]> {
  ...
  if (search.provider !== "DUCKDUCKGO_HTML" && search.provider !== "DUCKDUCKGO_LITE") return [];
  const now = new Date().toISOString();
  return search.results.slice(0, 10).flatMap((item, index) => {
    const domain = normalizeDomain(item.link);
    if (!domain || domain.includes("duckduckgo.com") || !isParisTravelResult(item, domain)) return [];
    return [{ keyword, theme, rank: index + 1, title: item.title.slice(0, 220), url: item.link.slice(0, 1000), domain, resultType: classifyDomain(domain), visibilityWeight: rankVisibility(index + 1), source: item.provider, status: "ESTIMATED" as const, observedAt: now }];
  });
}
...
function aggregateTheme(theme: string, results: DestinationSerpResult[]): DestinationThemeCapture {
  ...
  for (const result of relevant) {
    ...
    current.appearances += 1; current.bestRank = Math.min(current.bestRank, result.rank); current.weight += result.visibilityWeight; byDomain.set(result.domain, current);
  }
  ...
  const source: DestinationSearchSource = relevant[0]?.source ?? "NONE";
  return { theme, queryCount: new Set(relevant.map((result) => result.keyword)).size, resultCount: relevant.length, domains: topDomains.map((item) => ({ domain: item.domain, resultType: item.resultType, visibilityShare: item.visibilityShare, status: "ESTIMATED", source: `${source}_RANK_WEIGHT` })), topDomains: topDomains.slice(0, 10), status: relevant.length ? "ESTIMATED" : "UNKNOWN", source, observedAt: relevant[0]?.observedAt ?? new Date().toISOString() };
}
...
export async function collectDestinationCapture(universe: KeywordUniverse = parisUncoveredUniverse, budget: DestinationCaptureBudget | number = {}) {
  ...
  const providers = [...new Set(results.map((result) => result.source))];
  return { universe: universe.id, source: providers.length === 1 ? providers[0] : providers.length ? "DUCKDUCKGO_MIXED" : "NONE", measurement: "ESTIMATED_SERP_VISIBILITY" as const, note: "Bounded public SERP visibility uses DuckDuckGo HTML/Lite only. Missing provider results remain UNKNOWN; no legacy RSS fallback is used. Visibility share is rank-weighted and is not click share or traffic volume.", keywordCount: new Set(results.map((result) => result.keyword)).size, resultCount: results.length, results, themes };
}
