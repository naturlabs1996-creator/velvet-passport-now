...
function haversineMeters(a: ResearchLead, b: ResearchLead) {
  if (![a.lat, a.lon, b.lat, b.lon].every((value) => typeof value === "number" && Number.isFinite(value))) {
...
function matchScore(a: ResearchLead, b: ResearchLead) {
  ...
  const addressMatch = sameAddress(a.address, b.address);
  const distance = haversineMeters(a, b);
  const geoMatch = distance <= 120;
  const strongGeo = distance <= 40;

  let score = nameScore * 60;
  if (addressMatch) score += 30;
  if (geoMatch) score += 20;
  if (strongGeo) score += 10;
  if (normalizeText(a.name) === normalizeText(b.name)) score += 25;
...
function shouldMerge(a: ResearchLead, b: ResearchLead) {
  if (a.pageId !== b.pageId || a.theme !== b.theme) return false;
  const match = matchScore(a, b);
  if (normalizeText(a.name) === normalizeText(b.name) && normalizeText(a.name).length >= 5) return true;
  if (match.addressMatch && match.nameScore >= 0.35) return true;
  if (match.geoMatch && match.nameScore >= 0.45) return true;
  if (match.nameScore >= 0.82) return true;
  return false;
}
...
function leadEvidence(lead: ResearchLead): ResearchEvidence {
  return {
    ...
    claims: lead.rawClaims,
    independentKey: lead.independentKey,
  };
}

function chooseCanonicalName(leads: ResearchLead[]) {
  const official = leads.find((lead) => lead.sourceType === "OFFICIAL");
  if (official) return official.name;
  const map = leads.find((lead) => lead.sourceType === "MAP" && lead.name.length <= 90);
...
function mergeConfidence(leads: ResearchLead[]): { confidence: MergeConfidence; reasons: string[] } {
  ...
  if (sources.size >= 2 || (hasGeo && hasAddress)) return { confidence: "MEDIUM", reasons };
...
export function buildCandidatePortfolio(collections: Array<{ leads: ResearchLead[] }>) {
  return collections.map((collection) => ({
    candidates: normalizeAndMergeLeads(collection.leads),
  }));
}
