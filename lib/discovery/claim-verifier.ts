...
function evidenceSupportsClaim(evidence: ResearchEvidence, claim: string, type: ClaimType) {
  return evidence.claims.some((sourceClaim) => {
    const score = similarity(sourceClaim, claim); const source = normalize(sourceClaim); const target = normalize(claim);
    if (["HOURS", "PRICE", "ACCESS", "LOCATION"].includes(type)) {
      const targetNumbers = numbers(claim); const sourceNumbers = numbers(sourceClaim);
...
function ageDays(evidence: ResearchEvidence, now: Date) {
  const date = evidence.publishedAt ?? evidence.observedAt; const time = new Date(date).getTime();
...
function polarity(text: string, type: ClaimType) {
  ...
  if (type === "HOURS") { if (/\bclosed\b|permanently closed|temporarily closed/.test(value)) return "CLOSED"; if (/\bopen\b|opening|daily/.test(value)) return "OPEN"; }
  if (type === "PRICE") { if (/\bfree\b|free admission|no charge/.test(value)) return "FREE"; if (/€|eur|euro|ticket|admission|price/.test(value) && numbers(value).length) return `PAID:${numbers(value).join(",")}`; }
  if (type === "ACCESS") {
...
function conflictingEvidence(evidence: ResearchEvidence[], type: ClaimType) {
  ...
  const values = evidence.flatMap((item) => item.claims.map((claim) => polarity(claim, type)).filter((value): value is string => Boolean(value)));
  const unique = [...new Set(values)];
  if (type === "HOURS") return unique.includes("OPEN") && unique.includes("CLOSED");
  if (type === "ACCESS") return (unique.includes("PUBLIC") && unique.includes("PRIVATE")) || (unique.includes("RESERVATION_REQUIRED") && unique.includes("NO_RESERVATION"));
  if (type === "PRICE") { const hasFree = unique.includes("FREE"); const paid = unique.filter((value) => value.startsWith("PAID:")); return (hasFree && paid.length > 0) || new Set(paid).size > 1; }
  return false;
}
export function verifyClaim(candidate: MergedCandidate, claim: string, now = new Date()): VerifiedClaim {
  ...
  const conflictDetected = conflictingEvidence(evidence, type); const reasons: string[] = [];
  let status: ClaimStatus = "UNVERIFIED";
  if (!evidence.length) reasons.push("No evidence source directly supports this claim.");
    ...
    if (independentSources >= 2 && preferredSourcePresent && currentEvidencePresent) { status = "VERIFIED"; reasons.push("High-risk claim has two independent sources, current evidence and a source preferred for this fact domain."); }
    else if (independentSources >= 1) { status = "PARTIAL"; reasons.push("High-risk claim has support but lacks the full independence, freshness or source-authority requirement."); }
  } else if (risk === "MEDIUM") {
    if ((independentSources >= 2 && currentEvidencePresent) || (officialSourcePresent && !staleEvidenceOnly)) { status = "VERIFIED"; reasons.push("Medium-risk claim has corroborated current support or non-stale official support."); }
    else if (independentSources >= 1) { status = "PARTIAL"; reasons.push("Medium-risk claim has only limited corroboration."); }
  } else if (independentSources >= 1 && !staleEvidenceOnly) { status = "VERIFIED"; reasons.push("Low-risk factual claim has at least one usable supporting source."); }
