...
export async function GET() {
  ...
  return Response.json({
    ...
    configuration: { checks, criticalReady, providerReady, readyForMajorTests: criticalReady && providerReady, viatorMode: viatorMode === "sandbox" || viatorMode === "production" ? viatorMode : "unconfigured" },
    health: {
      ...
      components: health.components.map((item) => ({ component: item.component, status: item.status, degraded: item.degraded, fallbackActive: item.fallbackActive, primaryCode: item.primaryCode })),
    },
  }, { status: health.status === "red" ? 503 : 200, headers: { "Cache-Control": "private, no-store" } });
}
